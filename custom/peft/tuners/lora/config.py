...
class LoraConfig(PeftConfig):
    ...
    route_input_dim: int = field(
        default=0,
        metadata={
            "help": (
                "If > 0, lora_route takes an externally supplied embedding of this "
                "dimension (e.g. a task/type embedding set via layer.type_emb) instead "
                "of the layer's own input activations. 0 means route on the layer's "
                "own input (token-wise, the original behaviour)."
            )
        },
    )
